    ...
    <>
      <section id='sec6' className='w-full max-w-7xl m-auto bg-white py-24'>
        ...
        <div className='flex flex-wrap mt-11 justify-center items-center gap-5'>
          <a className={`font-medium cursor-pointer hover:text-[#1d4ed8] text-xs ${current === ? "" : "text-gray-600"}`} onClick={() => {setWorks(worksData), setCurrent(0)}}>ALL</a>
          <a className={`font-medium cursor-pointer hover:text-[#1d4ed8] text-xs ${current === ? "" : "text-gray-600"}`} onClick={() => filterer(1)}>LOGO</a>
          <a className={`font-medium cursor-pointer hover:text-[#1d4ed8] text-xs ${current === ? "" : "text-gray-600"}`} onClick={() => filterer(2)}>GRAPHIC</a>
          <a className={`font-medium cursor-pointer hover:text-[#1d4ed8] text-xs ${current === ? "" : "text-gray-600"}`} onClick={() => filterer(3)}>PRINT</a>
        </div>
