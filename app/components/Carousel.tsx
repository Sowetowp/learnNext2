        ...
        <>
            <main className='xl:h-screen 2xl:h-[40vh] h-[60vh] bg-black m-auto w-full overflow-hidden max-w-7xl relative'>
                {slides.map((slide, index) => (
                    <div key={index} className={`h-full w-full absolute ${currentIndex === index ? "z-[12]": prevIndex.current === index ? "z-[11]" :index === slides.length - 1 && currentIndex === 0 ||index === slides.length - 1 && currentIndex === 0? "-z-10": "z-10"} transition-transform duration-700`} style={{ transform: `translateX(${position(index)})` }}>
                ...
                <button onClick={handleNext} className='absolute z-20 md:right-5 right-0 h-full'>
