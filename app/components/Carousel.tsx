        ...
        <>
            <main className='h-screen bg-black m-auto w-full max-w-7xl relative'>
                {slides.map((slide, index) => (
                    <div key={index} className={`h-full w-full absolute ${currentIndex === index ? "z-[11]" : currentIndex === slides.length - 1 && index === 0 ? "-z-10 opacity-0" : index === slides.length - 1 && currentIndex === 0 ? "-z-10 opacity-0" : "z-10"} opacity-100 transition-transform duration-500`} style={{ transform: `translateX(${position(index)})` }}>
