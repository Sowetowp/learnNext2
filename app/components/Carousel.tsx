        ...
        <>
            <main className='h-screen bg-black m-auto w-full max-w-7xl relative'>
                {slides.map((slide, index) => (
                    <div key={index} className={`h-full w-full absolute ${currentIndex === index ? "z-[11]" : position(index) === "-100%" ? "-z-10" : position(index) !== "-100%" ? "-z-10" : "z-10"} transition-transform duration-500`} style={{ transform: `translateX(${position(index)})` }}>
